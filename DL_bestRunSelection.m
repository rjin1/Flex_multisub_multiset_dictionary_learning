function [RunCorrT, BestZ, BestZIndex, IndexAssign, Tmaps] = DL_bestRunSelection(Zs)
%% Select best run based on MST.
% Inputs:
% 1. Zs - cell, DL sparse coding coefficients of R runs where R = length of Z
% 2. data - Data of dimensions components x voxels x subjects
%
% Outputs:
% RunCorrT - Correlation metric
% BestZ - The most stable Z in Zs.
% BestZIndex - The index of the BestZ in Zs.
%b
...
numOfRuns = length(Zs);
numOfC = size(Zs{1}, 1);
numOfVoxels = size(Zs{1}, 2);

[Tmaps, ZsAligned, IndexAssign] = TmapsMultiRuns(Zs);

RunCorrT = zeros(numOfC, numOfRuns);
for k = 1:numOfRuns
    RunCorrT(:, k) = diag(abs(icatb_corr(ZsAligned{k}', Tmaps')));
end
...
plot(sum_RunCorrT);
xlabel("Run Index");
ylabel("Sum of Correlation Coefficients of Maps With T-map");
...
function [Tmaps, ZsAligned, IndexAssign] = TmapsMultiRuns(Zs)
    % % Components from each are clustered
    [IndexAssign, ZsAligned] = ComponentAlignment(Zs);
    ...
    for k = 1:NumRun
    %     sum(isnan(ZsAligned{k}(:)))
        ZsAligned{k} = icatb_zscore(ZsAligned{k}')';
        ZsAligned{k}(isnan(ZsAligned{k}))=0;
    end
...
function [IndexAssign, Zs] = ComponentAlignment(Zs)
    % Build a weight graph, build an MST, find a central node in MST, 
    % IndexAssign: 
    NumRun = length(Zs);
    NumComp = size(Zs{1}, 1);

    R = nchoosek(1:NumRun,2); % b = nchoosek(n,k) returns the binomial coefficient
    cost = zeros(1, size(R, 1));
    assignment = cell(1, size(R, 1));
    for i=1:size(R, 1) 
        corrM = icatb_corr(Zs{1,R(i,1)}', Zs{1,R(i,2)}'); %With sign correction 
        %corrM = abs(icatb_corr(Zs{1,C(i,1)}', Zs{1,C(i,2)}'));
        corrM(corrM > 1) = 1;
        [assignment{1,i}, cost(1,i)] = assignmentoptimal(1 - corrM);
        %BUG:RESOLVED(unequal component sizes present in input (Zs).
    ...
    %     Zs{k}(find(tmpInd==0),:)=[]; %temp fix 2
        Zs{k} = Zs{k}(tmpInd, :);

        Zs{k}(zeroInd,:)=zeros(size(zeroInd,1),size(Zs{1},2)); % assign zeros to those components
    end
